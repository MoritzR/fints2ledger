...
"""
    ...
    def setup_files(self):
        ...
        if not os.path.exists(self.getConfigFilePath()):
            with open(self.getConfigFilePath(), 'w') as config_file:
                config_file.write(
                    yaml.dump(Config.DEFAULT_CONFIG, default_flow_style=False)
                )
    ...
    def moveLocalAutoFilesIfPresent(self):
        for auto_file in glob.glob('*.auto'):
            ...
            print("I moved your '{}' to '{}'".format(
                auto_file, self.files_path))
    ...
    def get_config(self):
        ...
        parser.add_argument(
            ...
            help='file to store/load csv transactions to/from (default: transactions.csv)',
        )
        parser.add_argument(
            '--ledger-file',
            dest='ledgerfile',
        ...
        parser.add_argument(
            ...
            help='directory to store fints2ledger files (like config.yml) (default: ~/.config/fints2ledger/)',
        )
        parser.add_argument(
            '--date',
            dest='start',
            action='store',
            default=None,
            help='start date to pull the FinTS entries from (format: 2017/12/31 or 17/12/31, default: last year)',
        )
        parser.add_argument(
            '--separator',
            dest='separator',
